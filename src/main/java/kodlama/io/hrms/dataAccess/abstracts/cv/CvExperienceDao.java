...
public interface CvExperienceDao extends JpaRepository<CvExperience, Integer>{
	...
	/*@Query("from CvExperience where cv.jobSeeker.jobSeekerId = :jobSeekerId order by leaveDate asc nulls first ")
    List<CvExperience> getAllJobSeekerAndOrderedByAsc(int jobSeekerId);

    @Query("from CvExperience where cv.jobSeeker.jobSeekerId = :jobSeekerId order by leaveDate desc nulls first ")
